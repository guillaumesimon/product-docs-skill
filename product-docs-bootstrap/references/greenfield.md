...
- Expect every persona to be a guess, and write them as claims anyway. There are no users yet, so nothing on that page rests on evidence - mark each one `<span class="tag t-check">Guess</span>`, keep the page at `check`, and still name exactly one as the main persona. A founder who cannot say which person the thing is mainly for has not decided what they are building; asking is more useful on day one than at any later point. `personas.md` has the rest, and its warning about segmenting by screen applies double here, where the screens are imaginary.
