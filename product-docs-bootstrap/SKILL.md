...
None of it is in the repository, so it comes from an interview. Run it now, after the exploration and before writing: the Positioning explorer has just produced candidates, and people correct a wrong draft far better than they fill a blank form. Read `references/business-card.md` for the interview, the market research and the guard rails, and `references/personas.md` before the persona part of it - that page is the one an agent gets confidently wrong, and it fails by being full rather than empty.
...
**A spec** is one page per project, `<section class="page" data-parent="specs">`, with `data-spec-id`, `data-spec-status`, `data-objective`, `data-target`, `data-updated`. The index on the Specs page and the rows on the Roadmap read from it, so a project's name, status and objective are written once. Each one also carries a line saying how it serves the single objective, or why it is the exception - that line is what makes the roadmap arguable, and `data-objective` is the same answer in one word: acquisition, activation, retention, revenue, or `none` for a project that deliberately serves no objective, such as a regulatory constraint.
...
**A persona** is somebody outside the company - a role in the code is a permission, not a person - and the set is sorted by why people come, never by which screen they land on. Two people using different features for the same reason are one persona; two using the same feature for different reasons are two. Each one is a sketch that is never cut for being unsourced - first name, epithet, age range, two or three dispositions, a temperament emoji rather than a face - and a *Needs* line that is judged hard, which is what they are trying to get done and never a feature they asked for. The number falls out of how many reasons the product is picked up for, with a ceiling of five and no target, and exactly one is marked the main persona. Where nobody knows, write a claim marked `Guess`, never a question: a claim is corrected in one sentence, a question on the page is homework. `references/personas.md` has the rest.
...
**The roadmap** is one table in priority order. Each row points at its spec page with `data-spec` and sets `data-horizon`; rank, project name, objective and spec status are filled in automatically. Each objective has its own emoji and colour - 📈 acquisition, ⚡ activation, 🔁 retention, 💰 revenue, ⚪ no objective - and the one that matches the single objective is the only one drawn with a ring, so an off-objective project is visible at a glance. Two filters, by objective and by spec status, let the table be read one lever at a time. A row without a spec can still carry `data-objective` itself.
...
- `references/personas.md` - the personas page: the four ways it goes wrong, the shape, the picture set, the main persona, and the drafter-and-critic pair for when there is nobody to interview
