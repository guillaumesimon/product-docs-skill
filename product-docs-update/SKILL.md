...
**Roadmap rows.** One `<tr>` per live project inside `<tbody data-roadmap>`, in priority order, with `data-spec` pointing at the spec page and `data-horizon` set to now, next or later. Rank, project name, objective and spec status are filled in automatically - write only the problem, the metric and the horizon. A row for a project with no spec yet carries its own `data-objective`. Reordering the rows is how priority changes. A shipped or dropped project loses its row.

**A spec's objective.** Every spec page carries `data-objective`: the one lever it serves - acquisition, activation, retention or revenue - or `none` for a project that deliberately serves none, such as a regulatory constraint. It is the "Serves the objective" line in one word, so the two are written together and never left to disagree. The Roadmap turns it into a column - each objective with its own emoji and colour, the current one ringed - and filters on it alongside the spec status.
